...
local ain_brake    = analog:channel()

ain_throttle_fs:set_pin(4)
ain_throttle_hs:set_pin(13)
ain_brake:set_pin(8)
------------------------------------------------------------
-- MODE CONSTANTS
------------------------------------------------------------
...
local disarmed = 0
gcs:send_text(6, "IBR + HOLD + Geofence script loaded.")
...
local function rider_control_logic()
    ...
    local v_brake    = ain_brake:voltage_latest()
    ...
    if rider_in_control then
        -- Still in rider control mode; read inputs and output
        ...
        if arming:armed() == false then
            disarmed = 1
        end
        if disarmed == 1 and arming:armed() == true then
